...
import { GmDashboardService, PortfolioKpiResponse, ProjectKpiResponse, UpsertProjectRequest } from '../../services/gm-dashboard.service';
...
@Component({
  selector: 'app-my-cs-projects-page',
  templateUrl: './my-cs-projects-page.component.html',
  styleUrls: ['./my-cs-projects-page.component.scss']
})
export class MyCsProjectsPageComponent implements OnInit {
  ...
  loadInitialData(): void {
    ...
    this.gmDashboardService.getProjects('MY_CS').subscribe({
  ...
  openProject(projectId?: number | null): void {
    ...
    this.router.navigate(['/gm/my-cs/projects', projectId, 'schedule']);
  }
  ...
  saveProject(): void {
    ...
    const request$ = this.editingProjectId
      ? this.gmDashboardService.updateProject(this.editingProjectId, payload)
      : this.gmDashboardService.createProject(payload);
    ...
    request$.subscribe({
      ...
      error: () => {
        this.error = this.editingProjectId
          ? 'Failed to update MY CS project.'
          : 'Failed to create MY CS project.';
        this.saving = false;
      }
    });
  }
