...
export class CrmAccountsPageComponent implements OnInit {
  ...
  constructor(private crm:CrmService, private router:Router, public permissions:CrmPermissionsService, public countries:CrmCountryService, public i18n: CrmI18nService){}
  ...
  save():void{if(!this.form.name.trim()){this.error=this.i18n.t('accounts.error.requiredName');return;}this.saving=true;this.crm.createAccount(this.orgId,this.form).subscribe({next:v=>{this.saving=false;this.showForm=false;this.router.navigate(['/crm/accounts',v.id]);},error:e=>{this.error=this.message(e);this.saving=false;}});}
  ...
  private message(error:any):string{return error?.error?.message||this.i18n.t('accounts.error.unableToComplete');}
}