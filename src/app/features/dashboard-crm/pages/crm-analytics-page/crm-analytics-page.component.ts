...
export class CrmAnalyticsPageComponent implements OnInit {
  ...
  load(): void {
    this.loading = true; 
    this.error = '';
    this.crm.getAnalytics(this.crm.getOrgIdFromToken(), this.type || undefined)
  ...
  maxStage(): number { 
    return Math.max(1, ...(this.analytics?.pipelineByStage.map(value => value.count) || [0])); 
  }
  
  maxSource(): number { 
    return Math.max(1, ...(this.analytics?.leadsBySource.map(value => value.count) || [0])); 
  }
  ...
  expectedRevenue(item: CrmOpportunity): number { 
    return item.expectedRevenue ?? (item.value || 0) * (1 - (item.discount || 0) / 100) * item.probability / 100; 
  }
  
  openOpportunity(id: number | null): void { 
    if (id) this.router.navigate(['/crm/opportunities', id]); 
  }
}