...
export class CrmAccountDetailPageComponent implements OnInit{
 orgId=this.crm.getOrgIdFromToken();id=Number(this.route.snapshot.paramMap.get('id'));account?:CrmAccount;form?:CrmAccount;leads:CrmLead[]=[];opportunities:CrmOpportunity[]=[];users:CrmUser[]=[];industries:CrmIndustry[]=[];loading=true;editing=false;saving=false;error='';
