...
// 👇 IMPORT THE I18N SERVICE (Adjust the path if your service is located elsewhere)
...
export class CrmDashboardPageComponent implements OnInit {
  ...
  constructor(
    ...
    public i18n: CrmI18nService // 👈 ADDED: Expose i18n service to the HTML template
  ) {}

  get currentUserName(): string {
    return this.auth.getCurrentUsername() || 'there';
  }
  ...
  load(): void {
    this.loading = true;
    forkJoin({
      dashboard: this.crmService.getDashboard(this.orgId),
      opportunities: this.crmService.getOpportunities(this.orgId),
      stages: this.crmService.getStages(this.orgId),
      accounts: this.crmService.getAccounts(this.orgId)
    }).subscribe({
      next: result => {
        this.dashboard = result.dashboard;
        this.allOpportunities = result.opportunities;
        this.stages = result.stages;
        this.accounts = result.accounts;
        this.loading = false;
      },
      error: err => {
        this.error = err?.error?.message || 'Unable to load the CRM dashboard.';
        this.loading = false;
      }
    });
  }
  ...
  getOppsForStage(stage: CrmPipelineStage): CrmOpportunity[] {
    return this.allOpportunities.filter(opportunity => opportunity.stageId === stage.id && !opportunity.lost);
  }

  openNewOpp(): void {
    this.oppForm = emptyOpportunity();
    this.oppForm.stageId = this.stages[0]?.id ?? null;
    this.oppForm.probability = this.stages[0]?.probability ?? 0;
    this.oppForm.accountId = this.accounts[0]?.id ?? null;
    if (this.permissions.hasOwnOpportunityScope) {
      this.oppForm.ownerId = this.permissions.currentUserId;
    }
    this.oppError = '';
  ...
  saveNewOpp(): void {
    ...
    if (!this.oppForm.accountId) {
      this.oppError = 'Select an account.';
      return;
    }
    if ((this.oppForm.materialValue || 0) < 0 || (this.oppForm.servicesValue || 0) < 0) {
      this.oppError = 'Material and services cannot be negative.';
      return;
    }
    if (this.oppForm.discount < 0 || this.oppForm.discount > 100) {
    ...
    this.crmService.createOpportunity(this.orgId, this.oppForm).subscribe({
      ...
      error: err => {
        this.oppError = err?.error?.message || 'Unable to create the opportunity.';
        this.savingOpp = false;
      }
    });
  }
  ...
  sourceEntries(): { key: string; value: number }[] {
    return this.dashboard
      ? Object.entries(this.dashboard.leadsBySource).map(([key, value]) => ({ key, value })).sort((a, b) => b.value - a.value)
      : [];
  }
  ...
  sourceColor(source: string): string {
    return ({
      ...
      PARTNER: '#06b6d4',
      OTHER: '#94a3b8'
    } as Record<string, string>)[source] || '#94a3b8';
  }
  ...
  get totalLeads(): number {
    return this.dashboard ? Object.values(this.dashboard.leadsBySource).reduce((a, b) => a + b, 0) : 0;
  }

  getDonutSegments(): { color: string; offset: number; dash: number }[] {
    if (!this.dashboard || !this.totalLeads) return [];
    const circumference = 2 * Math.PI * 40;
    let offset = 0;
    return this.sourceEntries().map(source => {
      const dash = (source.value / this.totalLeads) * circumference;
      const segment = { color: this.sourceColor(source.key), offset: circumference - offset, dash };
      offset += dash;
      return segment;
    });
  }
